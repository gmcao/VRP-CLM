...
function [civ,cip,C]= cheapestIP( rv,rfvc,L,a,b,s,dist,demands,cap,speed)
...
for i=1:NV
    ...
    for j=1:len+1;
        ...
        if j==1
            ...
            [bs,back]= begin_s(temp_r,a,s,dist,speed );
...
if ~isempty(outcome)                            %检查是否存在合理的插入点
    addC=outcome(:,3);                          %提取 outcome 矩阵第三列的值，即每个插入点的距离增
    [saC,sindex]=sort(addC);                    %将距离增量从小到大排序，并获取排序后的索引 sindex
    temp=outcome(sindex,:);                     %根据排序后的索引重新排列 outcome 矩阵，得到新的 temp 矩阵，其中包含按距离增量从小到大排序的车辆序号、插入点序号和距离增量
    civ=temp(1,1);                              %从 temp 矩阵的第一行提取最佳插入点的车辆序号 civ
    cip=temp(1,2);                              %从 temp 矩阵的第一行提取最佳插入点的序号 cip
    C=temp(1,3);                                %从 temp 矩阵的第一行提取对应的距离增量 C
else                                            %如果 outcome 矩阵为空，表示没有合理的插入点。
    civ=NV+1;                                   %设置新的车辆序号 civ，这里 NV 是当前车辆的总数，所以 NV+1 表示新增一辆车
    cip=1;                                      %设置插入点序号为 1，通常表示车辆的起始点
    C=part_length(rv,dist);                     %计算新增车辆的总行驶距离 C
end
